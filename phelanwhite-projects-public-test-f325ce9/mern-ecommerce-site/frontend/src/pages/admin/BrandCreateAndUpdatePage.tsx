...
const BrandCreateAndUpdatePage = () => {
  const { id } = useParams();
  const { addBrand, updateBrandById, fetchBrandById } = useBrandStore();

  const getBrandByIdResult = useQuery({
    queryKey: ["brand", id],
    queryFn: async () => {
      try {
  ...
  const addAndUpdate = useMutation({
    ...
    mutationFn: async (data: any) => {
      ...
      if (window.location.pathname.includes("/create")) {
        ...
        result = await updateBrandById(id as any, data);
      }
      return result;
    },
    onSuccess(data) {
  ...
  useEffect(() => {
    if (window.location.pathname.includes("/update")) {
      setFormValue({ ...formValue, ...getBrandByIdResult.data });
    }
  }, [getBrandByIdResult.data]);

  if (addAndUpdate.isPending || getBrandByIdResult.isLoading) return <Loader />;
