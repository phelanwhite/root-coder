import { movies_options } from "@/assets/constants/commons";
import { IMAGE_MOVIE } from "@/assets/constants/images-constant";
import MediaCard from "@/components/common/MediaCard";
import Loader from "@/components/layout/loader";
import Paginate from "@/components/layout/paginate";
import useSearchParamsValue from "@/hooks/useSearchParamsValue";
import { moviesApi } from "@/services/tmdb-servies";
import { keepPreviousData, useQuery } from "@tanstack/react-query";

const MoviesPage = () => {
  const { searchParams, handleSearchParams } = useSearchParamsValue({
    search_type: movies_options[0].value,
    page: `1`,
  });
          ...
          <>
            <div className="grid gap-x-2 gap-y-6 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7">
              {search.data?.results?.map((item: any) => {
                return (
                  <MediaCard key={item?.id} data={item} media_type="movie" />
                );
              })}
            </div>
            <Paginate
