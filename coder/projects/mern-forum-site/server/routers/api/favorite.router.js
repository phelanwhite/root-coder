...
favoriteRouter.post(`/add-remove`, async (req, res, next) => {
  try {
    const body = req.body;
    const user = req.user;
    ...
    if (checkFavorite) {
      ...
      const newData = await favoriteModel.create({ ...body, user: user?._id });
      return handleResponse(res, {
...
favoriteRouter.delete(`/delete/:id`, async (req, res, next) => {
  try {
    const id = req.params.id;

    const deleteDta = await favoriteModel.findByIdAndDelete(id, {
      new: true,
    });
...
favoriteRouter.get(`/get-favorites-by-me`, async (req, res, next) => {
  try {
